...
class PicsView(View):
    ...
    def post(self, request):
        ...
        if pic_id:
            try:
                pic = PicsModels.objects.filter(name=name, wheel=wheel).exclude(id=pic_id)
            ...
            # 检验是否有结构的屏风图元
            try:
                pic = PicsModels.objects.filter(leftside=leftside, middle=middle, wheel=wheel, rightside=rightside).exclude(id=pic_id)
            except PicsModels.DoesNotExist:
                # 用户名不存在
                pic = None
            if pic:
