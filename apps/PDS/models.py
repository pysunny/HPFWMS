...
class Panelsets(BaseModel):
    ...
    FACE_STRUCTURE_CHOICES = (
        (0,'Skinned With Vertical Trim(固面带收边)'),
        (1,'Loose With Vertical Trim(易面带收边)'),
        (2,'Skinned Trimness(固面不带收边)')
    )
    SOUND_TEST_CHOICES = (
        (0,'No(不需要)'),
        (1,'Yes(需要)')
    ...
    location = models.CharField(max_length=64, null=True, verbose_name='区域名称')
    ...
    face_structure = models.SmallIntegerField(choices=FACE_STRUCTURE_CHOICES, null=True, verbose_name='屏风结构')
    ...
    decoration_text = models.CharField(max_length=256, null=True, verbose_name='装饰面文字说明')
    note = models.CharField(max_length=1024, null=True, verbose_name='备注信息')
    handle_quantity = models.SmallIntegerField(default=1, verbose_name='摇手柄数量')
    doorheight = models.CharField(max_length=64, null=True, verbose_name='门中门门高')
    lockeyoption_a = models.SmallIntegerField(choices=LOCKER_OPTION_CHOICES, null=True, default=None, verbose_name='A面锁类型')
    lockeyoption_b = models.SmallIntegerField(choices=LOCKER_OPTION_CHOICES, null=True, default=None, verbose_name='B面锁类型')
    pdsversion = models.ForeignKey('project.pdsVersion', default=1, on_delete=models.CASCADE, verbose_name='开工纸版本')
